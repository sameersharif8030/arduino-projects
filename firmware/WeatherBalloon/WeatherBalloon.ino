 ...
 *  FIXED: CORS Preflight (OPTIONS) + Cache Control Headers
 *  HTML Dashboard placed in separate file / placeholder.
 * ============================================================================
 */
...
//  HTML PLACEHOLDER
// ============================================================================
// REPLACE THIS STRING with your actual dashboard HTML content (from index_html[] PROGMEM)
// Tip: Use a tool to convert your HTML file to a C++ raw string literal (R"rawliteral(...)rawliteral")
// or keep it in a separate 'dashboard.h' and #include "dashboard.h"
...
void  handleData();
void  handleNotFound();
void  handleOptions();        // NEW: Explicit OPTIONS handler
...
void setup() {
  ...
  server.on("/data", HTTP_OPTIONS, handleOptions); // CRITICAL: Handle Preflight
  
  // Global fallback for any other OPTIONS requests
  server.onNotFound([]() {
  ...
  Serial.println(F("[SERVER] Dashboard at http://192.168.4.1\n"));
}
...
// ============================================================================
//  MPU6050 LOW LEVEL READ (Exact Wire.h method)
// ============================================================================
void readMPU(int16_t &AcX, int16_t &AcY, int16_t &AcZ) {
...
void readSensors() {
  ...
  { int medRaw = medianFilter(PIN_LM35, MEDIAN_SAMPLES);
    ...
    if (spikeReject(rawTemp, ema_temp, SPIKE_THRESH_TEMP, first_temp)) { first_temp = false; ema_temp = applyEMA(rawTemp, ema_temp, EMA_ALPHA_TEMP); }
  ...
  { int medRaw = medianFilter(PIN_MQ135, MEDIAN_SAMPLES);
    ...
    if (spikeReject((float)rawAqi, ema_aqi, (float)SPIKE_THRESH_AQI, first_aqi)) { first_aqi = false; ema_aqi = applyEMA((float)rawAqi, ema_aqi, EMA_ALPHA_AQI); }
  ...
  { int medRaw = medianFilter(PIN_MQ2, MEDIAN_SAMPLES);
    ...
    if (spikeReject((float)rawSmoke, ema_smoke, (float)SPIKE_THRESH_SMOKE, first_smoke)) { first_smoke = false; ema_smoke = applyEMA((float)rawSmoke, ema_smoke, EMA_ALPHA_SMOKE); }
  ...
  { int medRaw = medianFilter(PIN_MQ7, MEDIAN_SAMPLES);
    ...
    if (spikeReject((float)rawCO, ema_co, (float)SPIKE_THRESH_CO, first_co)) { first_co = false; ema_co = applyEMA((float)rawCO, ema_co, EMA_ALPHA_CO); }
  ...
  if (bmp180_ok) {
    ...
    if (rawPa > 0.0f) {
      ...
      if (spikeReject(rawPres, ema_pressure, SPIKE_THRESH_PRESSURE, first_pres)) { first_pres = false; ema_pressure = applyEMA(rawPres, ema_pressure, EMA_ALPHA_PRESSURE); }
      if (spikeReject(rawAlt, ema_altitude, SPIKE_THRESH_ALTITUDE, first_alt)) { first_alt = false; ema_altitude = applyEMA(rawAlt, ema_altitude, EMA_ALPHA_ALTITUDE); }
    }
  ...
  if (mpu6050_ok) {
    ...
    if (spikeReject(rawPitch, ema_pitch, SPIKE_THRESH_PITCH, first_pitch)) { first_pitch = false; ema_pitch = applyEMA(rawPitch, ema_pitch, EMA_ALPHA_PITCH); }
    if (spikeReject(rawRoll,  ema_roll,  SPIKE_THRESH_ROLL,  first_roll))  { first_roll  = false; ema_roll  = applyEMA(rawRoll,  ema_roll,  EMA_ALPHA_ROLL); }

    // Vibration uses raw averaged axes (pre-deadband) for sensitivity
...
// NEW: Explicit OPTIONS Handler for CORS Preflight
void handleOptions() {
...
void handleData() {
  // 1. Handle Preflight explicitly (belt-and-suspenders)
  if (server.method() == HTTP_OPTIONS) {
    handleOptions();
    return;
  }
  ...
  // 2. CORS: Echo Origin (Fixes wildcard issues on local network)
  String requestOrigin = server.header("Origin");
  server.sendHeader("Access-Control-Allow-Origin", requestOrigin.length() ? requestOrigin : "*");
  server.sendHeader("Access-Control-Allow-Credentials", "true");
  
  // 3. CACHE CONTROL: Force browser to re-fetch every time (Fixes stale data)
...
int multiSampleADC(uint8_t pin, uint8_t samples) {
  long sum = 0;
  for (uint8_t i = 0; i < samples; i++) { sum += analogRead(pin); delayMicroseconds(200); }
...
int medianFilter(uint8_t pin, uint8_t samples) {
  ...
  for (uint8_t i = 0; i < n; i++) buf[i] = multiSampleADC(pin, 8);
  for (uint8_t i = 1; i < n; i++) {
    int key = buf[i]; int8_t j = i - 1;
