...
// LSB per g at ±2g full-scale (MPU6050 default)
#define MPU_LSB_PER_G         16384.0f

// Vibration: magnitude deviation from 1g, scaled to integer 0-999
#define VIBRATION_SCALE       100.0f

// Deadband on raw int16 axes — same value used in working test sketch
#define MPU_DEADBAND          50

// Samples averaged per read cycle inside readMPU (matches working sketch: 20)
#define MPU_AVG_SAMPLES       20

// Samples for startup calibration (matches working sketch: 200)
...
<title>• Smart Weather Balloon Telemetry Dashboard</title>
...
// ============================================================================
//  MPU6050 CALIBRATION OFFSETS  (raw int16 — same type as working sketch)
// ============================================================================
long xOffset = 0;
long yOffset = 0;
long zOffset = 0;
...
// ============================================================================
//  readMPU()
//  Exact byte-by-byte Wire.h read from register 0x3B — copied verbatim
//  from the confirmed-working standalone test sketch.
// ============================================================================
void readMPU(int16_t &AcX, int16_t &AcY, int16_t &AcZ) {
...
// ============================================================================
//  setupMPU6050()
//  Wake sequence and calibration copied verbatim from working test sketch.
//  Offsets stored as long (same type as working sketch).
// ============================================================================
void setupMPU6050() {
  Serial.println(F("[MPU6050] Waking sensor..."));

  // Wake from sleep — identical to working test sketch
  Wire.beginTransmission(MPU_ADDR);
  Wire.write(0x6B);
  Wire.write(0);
  ...
  // Verify sensor is alive with a quick read
  ...
  // ── Calibration — same logic as working test sketch ──────────────────────
...
void readSensors() {
  ...
  // Uses exact same averaging + deadband logic as the confirmed working sketch.
  if (mpu6050_ok) {
    long sumX = 0, sumY = 0, sumZ = 0;
    int16_t AcX, AcY, AcZ;
    ...
    // Vibration: total magnitude deviation from 1g
    // Use raw (pre-deadband) averaged axes so small vibrations are not zeroed out
...
// ============================================================================
//  printTelemetry()  — every 3 seconds
// ============================================================================
void printTelemetry() {
  Serial.println(F("\n+------------------------------------------+"));
