 ...
 *           └── 10kΩ ──── GND
 *
 *  This halves the MQ output (5V → 2.5V max) keeping it within the
 *  ESP32 ADC's 3.3V limit. The code compensates with MQ_VD_SCALE = 2.
 * ============================================================================
 */
...
// ============================================================================
//  PIN DEFINITIONS  (change only here if you rewire)
// ============================================================================
#define PIN_LM35       33   // LM35  analogue output
#define PIN_MQ135      34   // MQ135 analogue output (input-only pin — OK)
#define PIN_MQ2        35   // MQ2   analogue output (input-only pin — OK)
#define PIN_MQ7        32   // MQ7   analogue output

#define PIN_I2C_SDA    21   // Shared I2C SDA — BMP180 + MPU6050
#define PIN_I2C_SCL    22   // Shared I2C SCL — BMP180 + MPU6050

#define GPS_ESP_RX     16   // ESP32 RX2 ← GPS TX
#define GPS_ESP_TX     17   // ESP32 TX2 → GPS RX  (mostly unused)
...
// --- MQ voltage-divider compensation ---
// 10kΩ / 10kΩ divider halves the signal; multiply raw ADC by 2 to restore.
#define MQ_VD_SCALE         2

// --- LM35 ---
// Output: 10 mV/°C.  ADC: 0–4095 = 0–3.3 V
// Temp(°C) = (ADC / 4095.0) × 3300 mV / 10 mV
// Trim this offset (°C) to match a reference thermometer if needed.
#define LM35_OFFSET         0.0f

// --- MQ135  pseudo-AQI mapping ---
// Compensated ADC (0–4095)  →  AQI (0–500)
#define MQ135_AQI_MAX       500

// --- MQ7  CO ppm mapping ---
// Compensated ADC (0–4095)  →  ppm (0–1000)
#define MQ7_CO_PPM_MAX      1000

// --- MQ2  Smoke / LPG ppm mapping ---
// Compensated ADC (0–4095)  →  ppm (0–1000)
#define MQ2_SMOKE_PPM_MAX   1000

// --- MPU6050 acceleration scale ---
// Default ±2 g range → 1 g = 16384 LSB
#define MPU_LSB_PER_G       16384.0f

// Vibration = |total_accel − 1 g| × this scale → integer 0–999
...
//  Paste your complete HTML between the rawliteral markers below.
// ============================================================================
...
void setup() {
  ...
  // Init sensors, WiFi AP, and web server
  setupSensors();
  setupWiFiAP();
...
// ============================================================================
void loop() {
  // Feed GPS characters into the parser
...
void setupSensors() {
  ...
  if (bmp180.begin()) {
    ...
    Serial.println(F("[BMP180]  FAIL — Not detected on I2C. Check SDA/SCL wiring."));
    Serial.println(F("[BMP180]  Continuing — altitude/pressure will read 0."));
  }
  ...
  if (mpu6050.testConnection()) {
    ...
    Serial.println(F("[MPU6050] FAIL — Not detected on I2C. Check SDA/SCL wiring."));
    Serial.println(F("[MPU6050] Continuing — pitch/roll/vibration will read 0."));
  }

  // NEO-6M GPS
  Serial.println(F("[NEO-6M]  Listening on UART2. Waiting for GPS fix..."));
  Serial.println(F("[NEO-6M]  lat/lon/speed will be 0 until fix acquired."));

  // Analog sensors — no initialisation required
  Serial.println(F("[LM35]    Analogue pin GPIO33 — ready."));
  Serial.println(F("[MQ135]   Analogue pin GPIO34 — ready. (verify voltage divider!)"));
  Serial.println(F("[MQ2]     Analogue pin GPIO35 — ready. (verify voltage divider!)"));
  Serial.println(F("[MQ7]     Analogue pin GPIO32 — ready. (verify voltage divider!)"));
...
void readSensors() {
  ...
  if (bmp180_ok) {
    float rawPa = (float)bmp180.readPressure();           // Pa
    if (rawPa > 0.0f) {
      tele.pressure = rawPa / 100.0f;                     // hPa
      // ICAO barometric altitude formula
      tele.altitude = 44330.0f *
                      (1.0f - powf(tele.pressure / 1013.25f, 1.0f / 5.255f));
    }
  }

  // ── LM35 : Temperature ───────────────────────────────────────────────────
  {
    int   raw_lm35  = analogRead(PIN_LM35);
    float mV        = (raw_lm35 / 4095.0f) * 3300.0f;    // mV (3.3 V ref)
    tele.temperature = (mV / 10.0f) + LM35_OFFSET;       // 10 mV / °C
  }

  // ── MQ135 : Pseudo-AQI ───────────────────────────────────────────────────
  {
    int raw        = analogRead(PIN_MQ135);
    int compensated = constrain(raw * MQ_VD_SCALE, 0, 4095);
    tele.aqi       = map(compensated, 0, 4095, 0, MQ135_AQI_MAX);
  }
  ...
  // ── MQ2 : Smoke / LPG ────────────────────────────────────────────────────
  {
    ...
    int compensated = constrain(raw * MQ_VD_SCALE, 0, 4095);
    tele.smoke     = map(compensated, 0, 4095, 0, MQ2_SMOKE_PPM_MAX);
  }
  ...
  // ── MQ7 : Carbon Monoxide ────────────────────────────────────────────────
  {
    ...
    int compensated = constrain(raw * MQ_VD_SCALE, 0, 4095);
    tele.co        = map(compensated, 0, 4095, 0, MQ7_CO_PPM_MAX);
  }

  // ── NEO-6M GPS : Latitude, Longitude, Speed ──────────────────────────────
  // Only update when a fresh, valid fix is available.
  // On no-fix, previous values (or 0.0 from init) are preserved.
  if (gps.location.isValid() && gps.location.isUpdated()) {
    tele.latitude  = gps.location.lat();
    tele.longitude = gps.location.lng();
  }
  if (gps.speed.isValid() && gps.speed.isUpdated()) {
    tele.speed = (float)gps.speed.kmph();
  }
  ...
  if (mpu6050_ok) {
    ...
    // Convert raw to g  (default ±2 g range, 16384 LSB/g)
    float axg = ax / MPU_LSB_PER_G;
    float ayg = ay / MPU_LSB_PER_G;
    float azg = az / MPU_LSB_PER_G;

    // Pitch — forward / backward tilt (degrees)
    tele.pitch = atan2f(ayg, sqrtf(axg * axg + azg * azg)) * (180.0f / M_PI);

    // Roll — left / right tilt (degrees)
    tele.roll  = atan2f(axg, sqrtf(ayg * ayg + azg * azg)) * (180.0f / M_PI);

    // Vibration — deviation from 1 g (scaled to 0–999 integer)
    float mag = sqrtf(axg * axg + ayg * ayg + azg * azg);
    tele.vibration = (int)constrain(fabsf(mag - 1.0f) * VIBRATION_SCALE, 0.0f, 999.0f);
  }
}

// ============================================================================
//  handleRoot()  —  Serve the HTML dashboard
// ============================================================================
void handleRoot() {
...
// ============================================================================
//  handleData()  —  Return live telemetry as JSON
// ============================================================================
void handleData() {
